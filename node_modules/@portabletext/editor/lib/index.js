...
import { createKeyboardShortcut, undo, redo, code, underline, italic, bold } from "@portabletext/keyboard-shortcuts";
...
}), coreAnnotationBehaviors = [addAnnotationOnCollapsedSelection, preventOverlappingAnnotations, stripAnnotationsOnFullSpanDeletion], arrowDownOnLonelyBlockObject = defineBehavior({
  on: "keyboard.keydown",
  guard: ({
    snapshot,
    event
...
function buildBehaviorIndex(behaviors) {
  const global = [], namespaced = /* @__PURE__ */ new Map(), exact = /* @__PURE__ */ new Map();
  let sortOrder = -1;
  for (const behavior of behaviors) {
    sortOrder++;
    const sortedBehavior = {
      behavior,
      sortOrder
    };
    if (behavior.on === "*") {
    ...
    if (behavior.on.endsWith(".*")) {
      const namespace = behavior.on.slice(0, -2), indexedBehaviors2 = namespaced.get(namespace) ?? [];
      indexedBehaviors2.push(sortedBehavior), namespaced.set(namespace, indexedBehaviors2);
      continue;
    }
    const indexedBehaviors = exact.get(behavior.on) ?? [];
    indexedBehaviors.push(sortedBehavior), exact.set(behavior.on, indexedBehaviors);
  }
  return {
    exact,
    global,
...
function getEventBehaviors(behaviorIndex, type) {
...
function performEvent({
  ...
  const eventBehaviors = mode === "forward" && forwardFromBehaviors ? forwardFromBehaviors : getEventBehaviors(mode === "execute" ? abstractBehaviorIndex : behaviorIndex, event.type);
      ...
      withoutNormalizingConditional(editor, () => raiseGroup || executeGroup, () => {
        for (const action of actions) {
          ...
          if (action.type === "forward") {
            const remainingBehaviors = eventBehaviors.slice(eventBehaviorIndex + 1);
            performEvent({
...
const editorMachine = setup({
  ...
  actions: {
    ...
    "index behaviors": assign(({
      context
    }) => {
      if (context.behaviorsIndexed)
        return {};
      const allBehaviors = [...sortByPriority([...context.behaviors.values()]).map((config) => config.behavior), ...abstractBehaviors];
      return {
  ...
  context: ({
    ...
    behaviorIndex: buildBehaviorIndex([...coreBehaviorsConfig.map((config) => config.behavior), ...abstractBehaviors]),
    abstractBehaviorIndex: buildBehaviorIndex(abstractBehaviors),
    behaviorsIndexed: !1,
